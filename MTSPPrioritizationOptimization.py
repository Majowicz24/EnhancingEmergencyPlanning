...
def mtsp_eval(individual):
    ...
    for traveler_id in range(4):  
        # Find indices for the current traveler's route
...
for traveler_id in range(4):
    ...
    priority_indices = [i for i in ordered_sub_indices if homes_nodes[i] in homes_to_visit]
    secondary_indices = [i for i in ordered_sub_indices if homes_nodes[i] not in homes_to_visit]

    # Calculate distance and time for the priority part
    priority_distance = 0
    time_stops_priority = (len(priority_indices) - 1) * STOP_DURATION
    ...
    # Store the priority distances and times
    priority_distances.append(priority_distance)
    priority_times.append(total_time_priority)

    # Concatenate the priority homes with the secondary homes to visit them in order 
    ordered_sub_indices = priority_indices + secondary_indices
